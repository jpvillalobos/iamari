...
export function StepCard({ children }: { children: ReactNode }) {
  return (
    <section className="relative bg-ink/60 border border-border-warm rounded-2xl p-6 sm:p-10 backdrop-blur-sm step-enter">
      {children}
    </section>
  );
}
...
export function StepHeader({
  ...
  return (
    <header className="mb-8">
      ...
      <h2 className="font-display text-3xl sm:text-4xl leading-[1.15] tracking-tight text-cream mb-3">
        {title}
      </h2>
      {subtitle && (
        <p className="text-text-muted leading-relaxed font-light max-w-xl">{subtitle}</p>
      )}
    </header>
  );
}
